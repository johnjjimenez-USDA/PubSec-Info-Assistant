...
class ChatReadRetrieveReadApproach(Approach):
    ...
    async def run(self, history: Sequence[dict[str, str]], overrides: dict[str, Any], citation_lookup: dict[str, Any], thought_chain: dict[str, Any]) -> Any:
        ...
        authors_filter = overrides.get("selected_authors", "")
        years_filter = overrides.get("selected_years", "")
        ...
        if authors_filter != "":
            if search_filter is not None:
                search_filter = search_filter + \
                    f" and authors/any(t: search.in(t, '{authors_filter}', ';'))"
            else:
                search_filter = f"authors/any(t: search.in(t, '{authors_filter}', ';'))"

        if years_filter != "":
            if search_filter is not None:
                search_filter = search_filter + \
                    f" and search.in(year, '{years_filter}', ',')"
            else:
        ...
        for idx, doc in enumerate(r):  # for each document in the search results
            ...
            citation_lookup[f"File{idx}"] = {
                "source_folder": doc[self.folder_field],
                "citation_content": doc[self.citation_field],
                ...
                "page_number": str(doc[self.page_number_field][0] if len(doc[self.page_number_field]) > 0 else 0) or "0",
            }
    ...
    def detect_language(self, text: str) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                raise Exception(
                    f"Error detecting language: {response.status_code} - {response.text}")
        except Exception as e:
            raise Exception(
    ...
    def translate_response(self, response: str, target_language: str) -> str:
        ...
        response = requests.post(
            api_translate_endpoint, headers=headers, json=data, params=params)
        ...
        if response.status_code == 200:
            ...
            raise Exception(
                f"Error translating response: {response.status_code}")
